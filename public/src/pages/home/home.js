...
import { GET_FILTER_LIST } from 'constants/api';
...
class HomeIndex extends Component {
	...
	handleClick = item => {
		this.props.history.push(
			`/home/detail?articleType=${item.articleType}&status=${item.status}&articleId=${item.aid}`
		);
	};
	...
	fetchData = (searchVal = '') => {
		...
		fireGetRequest(GET_FILTER_LIST, {
			page,
			articleType: pathname,
			searchVal,
			status: 1
		})
			.then(res => {
