...
class AdminInterfaceEdit extends Component {
		...
		this.props.form.validateFieldsAndScroll((err, values) => {
			...
			if (values.moduleType == '-1') {
				this.props.form.setFields({
					moduleType: { errors: [new Error('请选择模块')] }
				});
				return;
			}
			...
			firePostRequest(SAVE_INTERFACE, { ...params })
				.then(res => {
					if (res.code === 200) {
						showSuccessMsg('保存成功');
						fetchData(); // 重新获取左侧列表
	...
	handleConfirm = (key, type) => {
		...
		list[key] = {
			field: values['field_' + key + type],
			remark: values['remark_' + key + type],
			type: values['type_' + key + type],
			require: values['require_' + key + type]
		};
		if (type == 'req') {
			this.setState({
				fieldList: list,
	...
	getModuleList = () => {
		fireGetRequest(GET_INTERFACE_MODULE_LIST)
			.then(res => {
				if (res.code === 200) {
	...
	handleDelete = () => {
		...
		this.setState({ spinLoading: true });
		fireGetRequest(DELETE_INTERFACE_BY_ID, { id: selectedKeys })
			.then(res => {
				if (res.code === 200) {
					showSuccessMsg('删除成功');
					fetchData();
	...
	getEleList = type => {
		...
		return list.map((item, key) => {
			...
			return (
				<Row type='flex' gutter={20} className={'columnBody'} key={key}>
					<Col className={'col20'}>
						<FormItem label=''>
							{isEditNow
								? getFieldDecorator('field_' + key + type, {
										rules: [
											{
												required: true,
												message: '请输入字段名!'
					...
					<Col className={'col20'}>
						<FormItem label=''>
							{isEditNow
								? getFieldDecorator('remark_' + key + type, {
					...
					<Col className={'col15'}>
						<FormItem label=''>
							{isEditNow
								? getFieldDecorator('type_' + key + type, {
										rules: [
											{
												required: true,
												message: '请选择字段类型!'
					...
					<Col className={'col10'}>
						<FormItem label=''>
							{isEditNow
								? getFieldDecorator('require_' + key + type, {
										rules: [
											{
												required: true,
												message: '请选择是否字段必须!'
							...
							<Col>
								{isEditNow ? (
									...
									<Popconfirm
										title='是否确认删除?'
										onConfirm={() =>
											this.handleConfirmDel(key, type)
										}
										okText='是'
										cancelText='否'
									>
	...
	fetchData = () => {
		...
		let resFieldList = [];
		this.setState({ spinLoading: true });
		fireGetRequest(GET_INTERFACE_DETAILE_BY_ID, { id: selectedKeys })
			.then(res => {
				if (res.code === 200) {
